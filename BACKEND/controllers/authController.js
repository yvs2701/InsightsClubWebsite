    ...
    if (Object.keys(req.cookies).length !== 0 && req.cookies.hasOwnProperty('token') && req.body.hasOwnProperty('logout') && req.body.logout) {
        ...
        try {
            ...
            if (doc == null)
                ...
                const payload = { username: user.username, password: user.password, admin: doc.admin }
...
exports.signup = catchAsyncErrors(async (req, res, next) => {
    var usr = {}
    if (req.body.hasOwnProperty('username') && req.body.hasOwnProperty('name') && req.body.hasOwnProperty('email') && req.body.hasOwnProperty('password')) {
    ...
    try {
        user.save(async (err, doc) => {
            if (err && err.code === 11000) {
                return next(new ErrorHandler(`${Object.keys(err.keyPattern)[0]} is duplicate !!`, 409));
            } else if (err) {
                console.error(err);
                return next(new ErrorHandler('Some error occured !!', 500));
            }
...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        ...
        if (doc == null)
            ...
            res.status(200).json({ success: true, message: 'Email with link to reset password sent !!' });
        }
    } catch (e) {
        console.error(e);
        return next(new ErrorHandler('Some error occured !!', 500));
...
exports.reset = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        ...
        const doc = await User.findByIdAndUpdate(usr_id, { password: hashedPass }).lean();
        if (doc == null)
